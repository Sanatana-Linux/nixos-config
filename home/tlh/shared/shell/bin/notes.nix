...
create_or_open_note() {
    ...
    if [ -e "$NOTES_DIR/$filename" ]; then
        printf "Note with the same title already exists. Opening the note...\n"
        "$EDITOR" "$NOTES_DIR/$filename"
    else
        # Create the note file
        printf "# %s\n\n" "$title" > "$NOTES_DIR/$filename"
...
# Main menu
printf "Welcome to the Markdown Notes Manager!\n"
printf "-------------------------------------\n"

while true; do
    printf "Select an option:\n"
    printf "1. Create or Open a note\n"
    printf "2. List all existing notes\n"
    printf "3. View a specific note\n"
