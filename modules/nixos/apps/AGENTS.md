...
## Purpose
System-level application modules. Each module declares an enable option under `modules.apps.<name>` and provides system configuration when enabled.
...
- Every module uses `options.modules.apps.<name>.enable` guarded by `mkIf`
