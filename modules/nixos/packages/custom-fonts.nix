...
with lib; {
  options.modules.packages.customFonts = {
    enable = mkEnableOption "Custom font collection and configuration";
  };

  config = mkIf config.modules.packages.customFonts.enable {
    fonts = {
      packages = with pkgs; [
        nerd-fonts.agave
        nerd-fonts.d2coding
