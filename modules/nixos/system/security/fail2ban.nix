...
with lib; let
  cfg = config.modules.system.security.fail2ban;
in {
  options.modules.system.security.fail2ban = {
  ...
  config = mkIf cfg.enable {
    ...
    modules.system.security.firewall.enable = mkDefault true;
    ...
    # Uses attribute path syntax (not attrset replacement) to merge
    # with the NixOS-generated service unit rather than replacing it.
    # The previous `systemd.services.fail2ban = { ... }` form stripped
    # ExecStart/ExecStop, making the service non-functional.
    systemd.services.fail2ban.after = ["firewall.service"];
    systemd.services.fail2ban.wants = ["firewall.service"];
  };
}
