...
with lib; let
  cfg = config.modules.system.security.firewall;
in {
  options.modules.system.security.firewall = {
  ...
  config = mkIf cfg.enable {
    ...
    boot.kernel.sysctl = {
      ...
      # Disable unprivileged BPF (moved from security/packages.nix).
      # Prevents unprivileged users from loading BPF programs, closing
      # a common privilege escalation vector.
